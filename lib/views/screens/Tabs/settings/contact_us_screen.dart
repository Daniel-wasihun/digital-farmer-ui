...
class ContactUsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final bool isSmallPhone = size.width < smallPhoneMaxWidth && size.width >= verySmallPhoneMaxWidth;
    final bool isMediumPhone = size.width < mediumPhoneMaxWidth && size.width >= smallPhoneMaxWidth;
    ...
    final double maxWidth = isLargeTablet
        ...
        : isTablet
            ? 500
            : size.width * 0.95;
    final double maxHeight = size.height * (isVerySmallPhone ? 0.7 : isSmallPhone ? 0.65 : 0.7); // Allow more height on very small
    ...
    final double titleFontSize = (baseTitleSize * scaleFactor).clamp(13.0, 20.0); // Lowered min title size
    final double platformTextSize = (basePlatformTextSize * scaleFactor).clamp(8.0, 12.0); // Lowered min platform text size
    final double closeIconSize = (20.0 * scaleFactor).clamp(16.0, 24.0); // Lowered min close icon size
    ...
    if (isVerySmallPhone) {
        ...
        crossAxisCount = 2;
    } else if (isMediumPhone) { // Medium phones and potentially small tablets if below 600
        crossAxisCount = 3;
    } else { // Larger phones (>500), tablets
        crossAxisCount = 4;
    }
    ...
    return Dialog(
      shape: RoundedRectangleBorder(
        borderRadius: BorderRadius.circular(math.max(8.0, 16.0 * scaleFactor)), // Min radius
      ),
      backgroundColor: Colors.transparent,
      insetPadding: EdgeInsets.symmetric(horizontal: cardPadding * 0.8, vertical: 24.0), // Adjusted horizontal inset slightly
      child: ConstrainedBox(
        constraints: BoxConstraints(
          maxWidth: maxWidth.clamp(240, 600), // Lowered min width constraint
          maxHeight: maxHeight.clamp(220, 450), // Lowered min height constraint
        ),
        child: Stack(
          children: [
            Card(
              ...
              shape: theme.cardTheme.shape ?? RoundedRectangleBorder(
                 borderRadius: BorderRadius.circular(math.max(8.0, 16.0 * scaleFactor)), // Match dialog shape
              ),
                  ...
                  children: [
                    Padding(
                      padding: EdgeInsets.only(
                          top: closeIconSize * 0.5,
                          left: closeIconSize * 1.5,
                          right: closeIconSize * 1.5,
                    ...
                    Flexible(
                      child: SingleChildScrollView( // Keep for safety, though less likely needed now
                        child: GridView.count(
                          ...
                          padding: EdgeInsets.symmetric(horizontal: cardPadding * 0.2, vertical: gridSpacing * 0.5), // Reduced grid padding
                          children: [
                            _buildSocialLink(
                              context,
                              ...
                              axis: crossAxisCount == 1 ? Axis.horizontal : Axis.vertical, // Pass axis
                            ),
                            _buildSocialLink(
                              context,
                              icon: Icons.send,
            ...
            Positioned(
              top: cardPadding * 0.3, // Fine-tune position
              right: cardPadding * 0.3,
              child: IconButton(
                icon: Icon(
                  Icons.close,
                ...
                style: IconButton.styleFrom(
                  minimumSize: Size(closeIconSize * 1.8, closeIconSize * 1.8),
                ),
              ),
            ),
          ],
  ...
  Widget _buildSocialLink(
    ...
    final theme = Theme.of(context);
    final bool isDarkMode = theme.brightness == Brightness.dark;
    final double itemInternalPadding = (4.0 * scaleFactor).clamp(2.0, 5.0); // Reduced min/max
    final double iconContainerPadding = (5.0 * scaleFactor).clamp(3.0, 7.0); // Reduced min/max
    final double iconBorderRadius = (7.0 * scaleFactor).clamp(5.0, 9.0); // Reduced min/max
    final double spacingBetweenElements = itemInternalPadding * (axis == Axis.horizontal ? 2.5 : 1.5); // More spacing for Row
    ...
    Widget iconWidget = Material(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(iconBorderRadius),
          border: Border.all(
            color: theme.colorScheme.secondary.withOpacity(isDarkMode ? 0.4 : 0.25),
    ...
    Widget textWidget = Flexible( // Use Flexible within Row/Column
       // No flex needed if Column, but helps in Row
      ...
      child: Text(
        ...
        textAlign: axis == Axis.horizontal ? TextAlign.start : TextAlign.center, // Align text differently
        maxLines: 2,
        overflow: TextOverflow.ellipsis,
      ),
    );
    ...
    Widget content = (axis == Axis.horizontal)
        ? Row(
            mainAxisSize: MainAxisSize.min, // Row takes minimum horizontal space needed
        ...
        child: InkWell(
          ...
          child: Padding(
            ...
            padding: axis == Axis.horizontal
                ? EdgeInsets.symmetric(horizontal: itemInternalPadding * 2, vertical: itemInternalPadding)
                : EdgeInsets.all(itemInternalPadding * 0.8),
            child: Center(child: content), // Center the Row/Column within the InkWell area
          ),
        ),
      ),
    );
